function revog {
    ...
    $extractCmd = "& `"$sevenZipPath`" x `"$sourceZip`" -o`"$extractPath`" -p123 -y"
    ...
    $licExtractCmd = "& `"$sevenZipPath`" x `"$licRarPath`" -o`"$revoFolder`" -p123 -y"
    Invoke-Expression $licExtractCmd
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Failed to extract lic.rar with password, trying without password..."
        $licExtractCmd = "& `"$sevenZipPath`" x `"$licRarPath`" -o`"$revoFolder`" -y"
        Invoke-Expression $licExtractCmd
        if ($LASTEXITCODE -ne 0) {
    ...
    if ($appPath -eq $null) {
        # Try searching through Start Menu shortcuts
        try {
